...
class BiLSTM_CRF_Tagger:
    ...
    def extract_sentence_embeddings(self, texts: List[str]) -> np.ndarray:
        """Extracts pooled BiLSTM sentence embeddings (N, hidden_dim) for downstream Classic ML Regressors/Classifiers."""
        if self.model is None:
    ...
    def prepare_token_feature_cache(self, train_df: pd.DataFrame, test_df: pd.DataFrame) -> Dict[str, Any]:
        """
        Extracts token-level BiLSTM representations for all tokens in train and test datasets,
        enabling Classical Classifiers (XGB, RF, Logistic, etc.) to perform BIO token classification.
        """
        if self.model is None:
            raise ValueError("BiLSTM_CRF_Tagger must be fitted before preparing token features.")
        self.model.eval()
